...
export async function activate(_context: ExtensionContext) {
	...
	commands.registerCommand('testbed-dotnet.run', async () => {
		...
		const terminal = window.createTerminal({ name: 'DOTNET-WASI', pty, isTransient: true });
		terminal.show(true);
		...
		const module = await WebAssembly.compile(await fs.readFile(path.join(__dirname, 'dotnet-wasi-hello.wasm')));
